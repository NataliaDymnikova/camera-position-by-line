function eqi = generate_instances_for_eq_known_cs_equal_r( cfg, eq, known, unknown )
...
flag = true;
while flag
    R1_x = get_r_with_prime_known_cs_diff_r(prime);
    R1_yz = get_r_with_prime_known_cs_diff_r_yz(prime);
    R3_x = get_r_with_prime_known_cs_diff_r(prime);
    R3_yz = (R1_yz*R1_x)' * R3_x';
    R2_yz = get_r_with_prime_known_cs_diff_r(prime)';
    
    if mod(det(R1_x),prime) == 1 && mod(det(R3_x),prime) == 1
        flag = false;
    end
end
